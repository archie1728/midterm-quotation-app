...
function QuotationTable({ data, clearDataItems, deleteByIndex }) {
  ...
  const subtotal = data.reduce((acc, v) => acc + (v.qty * v.ppu), 0);
  ...
  return (
    <Container>
      ...
      <Table striped bordered hover>
          ...
          data.map((v, i) => {
            ...
            return (
              <tr key={i}>
                ...
                <td className={style.textCenter}>${v.discount.toFixed(2)}</td>
        ...
        <tfoot>
          <tr>
            <td colSpan={5} className={style.textRight}>Subtotal:</td>
            <td className={style.textRight}>${subtotal.toFixed(2)}</td>
          </tr>
          <tr>
            <td colSpan={5} className={style.textRight}>Total Discount:</td>
